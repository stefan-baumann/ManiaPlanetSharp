...
namespace ManiaPlanetSharp.GameBox.Parsing.ParserGeneration.AutoGenerated
{
	public static partial class AutoGeneratedParsers
	{
		public static Dictionary<Type, IParser<Chunk>> ChunkParsers { get; } = new Dictionary<Type, IParser<Chunk>> {
			...
			{ typeof(ManiaPlanetSharp.GameBox.Parsing.Chunks.Other.TestChunk), new TestChunkParser() },
		};
	}
	...
	public class TestChunkParser
		: PregeneratedChunkParser<ManiaPlanetSharp.GameBox.Parsing.Chunks.Other.TestChunk>
	{
		public override List<uint> ParseableIds => new List<uint>() { 0x12345678 };

        public override ManiaPlanetSharp.GameBox.Parsing.Chunks.Other.TestChunk Parse(GameBoxReader reader, uint chunkId)
        {
			var result = new ManiaPlanetSharp.GameBox.Parsing.Chunks.Other.TestChunk() { Id = reader.ReadUInt32() };
			if (result.ChunkId == 0)
			{
			    result.Ignored = reader.ReadString();
			}
