...
namespace ManiaPlanetSharp.GameBoxView
{
    public class DecorationTreeNode
        : TextTreeNode
    {
        public DecorationTreeNode(string name, string timeOfDay, string mappedTimeOfDay, string environment, string environmentAuthor, Size3D? size)
            : base(name, $"{timeOfDay} ({environment})")
        {
            this.TimeOfDay = timeOfDay;
            this.MappedTimeOfDay = mappedTimeOfDay;
            ...
            if (!string.IsNullOrWhiteSpace(this.TimeOfDay))
                this.Nodes.Add(new TextTreeNode("Mapped Time of day", this.MappedTimeOfDay));
            if (this.Size != null)
        ...
        public string MappedTimeOfDay { get; private set; }
